...
from hailtop.utils import flatten, parse_docker_image_reference
...
'''
    ...
    def cleanup(self, batch, scope, parents):
        ...
        if CLOUD == 'azure':
            ...
            image_name = parse_docker_image_reference(self.image).path
            script = f'''
set -x
date
...
until az acr repository delete --yes -n {shq(DOCKER_PREFIX)} --image {shq(image_name)} || ! az acr repository show -n {shq(DOCKER_PREFIX)} --image {shq(image_name)}
do
